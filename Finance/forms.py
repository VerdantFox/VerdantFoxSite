    ...
    def clean(self):
        ...
        if "symbol" in self.cleaned_data:
            ...
            if not symbol.isalpha():
                raise forms.ValidationError("Stock symbol must be alphabetical")
...
class AnalysisForm(forms.Form):
    ...
    time_frame = forms.ChoiceField(
        ...
        widget=forms.Select(attrs={"name": "time frame", "class": "form-control"}),
    )
...
class BuyForm(forms.Form):
    ...
    symbol = forms.CharField(
        label="Stock symbol:", max_length=5, widget=forms.HiddenInput()
    )

    buy_shares = forms.IntegerField(
        required=False,
        label="Purchase shares:",
        widget=forms.NumberInput(attrs={"name": "shares", "class": "form-control",}),
    )
...
class AddFundsForm(forms.Form):
    funds = forms.FloatField(
        ...
        widget=forms.NumberInput(
            attrs={"placeholder": "$25,000", "class": "form-control",}
        ),
    )
    ...
    card_number = CreditCardField(
        placeholder="0000 0000 0000 0000", min_length=12, max_length=19
    )
    ...
    zip_code = forms.CharField(
        ...
        max_length=10,
        widget=forms.NumberInput(
            attrs={"placeholder": "55555", "class": "form-control",}
        ),
    )
