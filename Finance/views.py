...
from .helpers import stock_index, search_stock_info, check_repeats, validate_shares
...
def ajax_stock_list(request):
    ...
    if isinstance(stock_list, str):
        return render(
            request, "Finance/ajax_stock_list.html", context={"error": stock_list}
        )
                ...
                if stock_symbol not in repeat_list and item[:3] == "buy":
                    ...
                    if request.POST[item]:
                        ...
                        valid_shares = validate_shares(
                            request, stock_symbol, request.POST[item]
                        )
                        if valid_shares:
...
def portfolio(request):
    ...
    return render(
        request, "Finance/portfolio.html", context={"quote_form": quote_form,}
    )
...
def ajax_graph(request):
    ...
    if request.method == "POST":
        ...
        if not div:
            messages.add_message(
                request, messages.WARNING, f"error building graph", extra_tags=" stock"
            )
            graph_error = script
            script = None
...
class HistoryView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        transactions = self.model.objects.filter(user=self.request.user).order_by(
            "-time_stamp"
        )
...
class AddFunds(LoginRequiredMixin, FormView, base.ContextMixin):
    ...
    def form_valid(self, form):
        ...
        user_info.save()
        messages.add_message(
            self.request, messages.SUCCESS, f"Added ${funds:,.2f} to your account!"
        )
